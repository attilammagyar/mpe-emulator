...
void ImportSettingsButton::import_settings(
...
bool ImportSettingsButton::mouse_move(
        int const x,
        int const y,
        bool const modifier
) {
...
bool ExportSettingsButton::mouse_move(
        int const x,
        int const y,
        bool const modifier
) {
...
void OptionSelector::add_option(unsigned int value, char const* const name)
{
    ...
    Option* const option = new Option(
...
void OptionSelector::select_option(
    ...
    Option* const selected_option = options_by_value[value];
    ...
    if (selected_option_index < OPTIONS_SIZE) {
        Option* const previously_selected_option = (
...
GUI::Image* ParamStateImages::free_images(GUI::Image* const images) const
{
    if (images == NULL) {
        return NULL;
    }
...
KnobParamEditor::KnobParamEditor(
    ...
    discrete_step_size(
        is_continuous
            ? 0.0
            : 1.001 / (double)proxy.get_param_max_value(param_id)
    ),
    knob_states(knob_states),
    value_font_size(is_continuous ? 22 : 20),
    knob_top(knob_top),
    has_room_for_text(
        height >= knob_top + knob_states->height + VALUE_TEXT_HEIGHT
    ),
    proxy(proxy),
    ratio(0.0),
    knob(NULL)
...
void KnobParamEditor::set_up(
        GUI::PlatformData platform_data, WidgetBase* const parent
) {
...
bool ToggleSwitchParamEditor::paint()
{
    ...
    double const scale = this->scale;

    double const scaled_left = scale * box_left;
    double const scaled_top = scale * 16.0;
    double const scaled_width = scale * 22.0;
    double const scaled_height = scale * 16.0;
...
{
    if (MPE_EMULATOR_LIKELY(gui != NULL)) {
        gui->start_resizing();
    }
...
{
    if (MPE_EMULATOR_UNLIKELY(gui == NULL)) {
        return;
    }
