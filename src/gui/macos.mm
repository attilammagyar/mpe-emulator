...
runtime complaining.
*/
#define MPE_EMULATOR_COCOA_WIDGET_CLASS             \
    _MPE_EMULATOR_OBJC_CLASS_NAME_RESOLVE_CONCAT(   \
        MPE_EMULATOR_OBJC_CLASS_NAME_INFIX          \
    )

#define _MPE_EMULATOR_OBJC_CLASS_NAME_RESOLVE_CONCAT(infix) \
    _MPE_EMULATOR_OBJC_CLASS_NAME_CONCAT(infix)

#define _MPE_EMULATOR_OBJC_CLASS_NAME_CONCAT(infix) \
    MPEEmulator_ ## infix ## _CocoaWidget


@interface MPE_EMULATOR_COCOA_WIDGET_CLASS : NSView
    @property (assign) MpeEmulator::Widget* cpp_widget;
    @property (strong) NSTimer* gui_idle_timer;
@end


@implementation MPE_EMULATOR_COCOA_WIDGET_CLASS
    ...
    - (void) drawRect:(NSRect)dirtyRect
    {
        ...
        MpeEmulator::Widget::notify_paint(self.cpp_widget);
    }
    ...
    - (void) scrollWheel:(NSEvent*)event
    {
        ...
        if (std::fabs(delta) > 0.000001) {
            MpeEmulator::Widget::notify_mouse_wheel(
                self.cpp_widget,
                delta,
    ...
    - (void) set_up_gui_idle_timer
    {
        self.gui_idle_timer = [
            NSTimer
            scheduledTimerWithTimeInterval:MpeEmulator::GUI::REFRESH_RATE_SECONDS
            repeats:YES
            block:^(NSTimer* timer) {
                if (self.cpp_widget) {
                    MpeEmulator::Widget::notify_timer_tick(self.cpp_widget);
                }
            }
        ];
    }
    ...
    - (void) handle_mouse_move:(NSEvent*)event :(BOOL)is_drag
    {
        ...
        MpeEmulator::Widget::notify_mouse_move(
            self.cpp_widget,
            (int)p.x,
            (int)p.y,
            ([event modifierFlags] & NSEventModifierFlagControl) != 0
...
MpeEmulator::GUI::PlatformWidget mpe_emulator_create_platform_widget(
    ...
    MPE_EMULATOR_COCOA_WIDGET_CLASS* cocoa_widget = [
        [MPE_EMULATOR_COCOA_WIDGET_CLASS alloc] initWithFrame:frame
    ];
    ...
    if (type == MpeEmulator::WidgetBase::Type::BACKGROUND) {
        [cocoa_widget set_up_gui_idle_timer];
    }

    return (__bridge_retained MpeEmulator::GUI::PlatformWidget)cocoa_widget;
}
...
void mpe_emulator_destroy_platform_widget(
    ...
    MPE_EMULATOR_COCOA_WIDGET_CLASS* cocoa_widget = (
        (__bridge_transfer MPE_EMULATOR_COCOA_WIDGET_CLASS*)platform_widget
    );
...
void mpe_emulator_widget_resize(
    ...
    MPE_EMULATOR_COCOA_WIDGET_CLASS* cocoa_widget = (
        (__bridge MPE_EMULATOR_COCOA_WIDGET_CLASS*)platform_widget
    );
...
void mpe_emulator_widget_fill_rectangle(
    ...
    CGFloat red = (CGFloat)MpeEmulator::GUI::red(color) / 255.0;
    CGFloat green = (CGFloat)MpeEmulator::GUI::green(color) / 255.0;
    CGFloat blue = (CGFloat)MpeEmulator::GUI::blue(color) / 255.0;
...
void mpe_emulator_widget_draw_text(
    ...
    NSColor* ns_color = [
        NSColor
        colorWithDeviceRed:(CGFloat)MpeEmulator::GUI::red(color) / 255.0
        green:(CGFloat)MpeEmulator::GUI::green(color) / 255.0
        blue:(CGFloat)MpeEmulator::GUI::blue(color) / 255.0
        alpha:1.0
    ];
    ...
    NSFont* ns_font = (
        font_weight == MpeEmulator::WidgetBase::FontWeight::BOLD
    ...
    switch (alignment) {
        case MpeEmulator::WidgetBase::TextAlignment::CENTER:
            style.alignment = NSTextAlignmentCenter;
            break;

        case MpeEmulator::WidgetBase::TextAlignment::RIGHT:
...
MpeEmulator::GUI::Image mpe_emulator_widget_load_image(char const* const name)
{
    ...
    NSString* path = [
        [NSBundle bundleForClass:[MPE_EMULATOR_COCOA_WIDGET_CLASS class]]
...
MpeEmulator::GUI::Image mpe_emulator_widget_copy_image_region(
    ...
    return (MpeEmulator::GUI::Image)CGImageCreateWithImageInRect(
        (CGImageRef)source,
        CGRectMake((CGFloat)left, (CGFloat)top, (CGFloat)width, (CGFloat)height)
    );
}
...
MpeEmulator::GUI::Image mpe_emulator_widget_downscale_image(
    ...
    return (MpeEmulator::GUI::Image)scaled_img;
}
